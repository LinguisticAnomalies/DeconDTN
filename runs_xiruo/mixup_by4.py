...
parser.add_argument(
    ...
    type=int,
    default=4,
    help="Number of mixup examples (times)",
)
...
# +
## No DistMatch, simply augup by 4
names_to_aug_ls = [
    "n_z0_pos_train",
    "n_z0_neg_train",
    "n_z1_pos_train",
...
for idx, (_, _df) in enumerate(zip(names_to_aug_ls, df_ls)):
    n_augs_dict[_] = c[_] * args.mixupTimes
...
augMethod = f"mixup-by{args.mixupTimes}"
    ...
    def forward(
        ...
        for k, v in self.n_aug_targets.items():
            if (v > 0) and (self.n_aug_tracker[k] < v):
                ...
                indexMixUp = (labels == aug_query_dict[k]["labels"]) & torch.tensor(
                    (np.array(domain_label.cpu()) == aug_query_dict[k]["domain_idx"])
                ).to(globalconfig.device)

                sequence_output_forMixUp = sequence_output[indexMixUp]

                all_combos = list(
                    itertools.combinations(range(sequence_output_forMixUp.shape[0]), 2)
                )
                ...
                for _combos in all_combos:
                    ...
                    sequence_output_stack.append(
                        _lambda * sequence_output_forMixUp[_combos[0]]
                        + (1 - _lambda) * sequence_output_forMixUp[_combos[1]]
                    )
                    _ct += 1
        ...
        if labels is not None:
            if len(sequence_output_stack) > 0:
                ...
                labels_mixup = torch.tensor(
                    np.hstack(labels_mixup_stack), dtype=torch.int64
                ).to(globalconfig.device)

                loss_mixup = loss_fct(
                    logits_mixup.view(-1, self.num_labels), labels_mixup.view(-1)
                )
...
model = RobertaForSequenceClassificationMixUp.from_pretrained(
...
writer = SummaryWriter(
    f"../output/testMixup-{dataset}/TestTB-AugBy4-exp-n_{globalconfig.batch_size}-mixupAlpha_{globalconfig.alpha}-lr_{globalconfig.lr}-WeightDecay_{globalconfig.weight_decay}-Momentum_{globalconfig.momentum}-nEpochs_{globalconfig.num_train_epochs}"
)
...
model.train()
for i_epoch in tqdm(
    range(globalconfig.num_train_epochs), file=open("../log/mixupTQDM.txt", "w")
):
    ...
    for i_step, batch in enumerate(train_loader):
        ...
        reducedBatch = {
            k: v.to(globalconfig.device) for k, v in batch.items() if k not in ["Text"]
        }
