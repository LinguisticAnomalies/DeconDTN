...
def datasets_loader(df, txt_col):
    ...
    ret_datasets = datasets.Dataset.from_pandas(
        df[[txt_col, "label"]].reset_index(drop=True)
    )
...
for combination in itertools.product(
    ...
    if (
        (number_setting is not None)
        and (number_setting["mix_param_dict"]["C_y"] in [0.2, 0.48, 0.72])
        and (number_setting["mix_param_dict"]["alpha_train"] in [1.0, 3, 5, 1 / 3, 0.2])
    ):
...
for ct, c in enumerate(tqdm(valid_full_settings)):
    ...
    for run_i in range(globalconfig.runs):
        ...
        globalconfig.output_dir = (
            f"/gscratch/scrubbed/xiruod/LoRA_BERT_SHAC/exp_{ct}_run_{run_i}"
        )
