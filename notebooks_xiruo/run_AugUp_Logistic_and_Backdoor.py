...
if args.constraintCy:
    ...
    for combination in itertools.product(
        ...
        if (
            round(combination[1] / combination[0], 4)
            not in SAMPLE_CONFIG[sample_select]["alpha_train"]
        ):  # [1, 0.2, 5]: # [1, 2, 0.5, 3, 0.3333, 4, 0.25, 5, 0.2]:
            continue

        ## select based on Cy
        if (
            round(
                combination[0] * (1 - combination[2]) + combination[1] * combination[2],
                4,
            )
            not in SAMPLE_CONFIG[sample_select]["C_y"]
        ):
            continue
...
outdir = f"../output/regression_Augmentation_{args.dataset}_BalanceAlpha"
            ...
            if args.augMethod != "noaug":
                ...
                def augDFReSample(df, name_to_aug):
                    ...
                    if n_augs < 0:
                        ...
                        df = pd.concat([df, _df], ignore_index=True).reset_index(
                            drop=True
                        )
