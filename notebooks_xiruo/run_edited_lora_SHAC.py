...
# args.adapterDir = "../output/tmpData/newAdapter/delta/"
adapter_name = "delta"
model = PeftModel.from_pretrained(model, args.adapterDir, adapter_name=adapter_name)


# ##### Load Model with Original Target Adapter
...
# model_originalTarget = LlamaForSequenceClassification.from_pretrained(
#     globalconfig.model_id,
#     device_map="auto",
#     load_in_8bit=args.quantization,
#     torch_dtype=torch.float16,
...
# model_originalTarget.config.pad_token_id = tokenizer.pad_token_id

# model_originalTarget.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=128)

# model_originalTarget = PeftModel.from_pretrained(
#     model_originalTarget, target_model_id, adapter_name="target"
# )
...
p_mix_z1_ls = np.arange(0.1, 0.1, 0.1)
...
for iRun in range(runs):
    ...
    for c in tqdm(valid_full_settings, file=open(log_f, "w")):
        ...
        with torch.no_grad():
            ...
            y_probs_lora = np.concatenate(y_ls)
        ...
        auprc_lora.append(
            metrics.average_precision_score(y_true=y_test, y_score=y_probs_lora[:, 1])
        )
        auprc_lora_df0.append(
            metrics.average_precision_score(
                y_true=y_test[df_test[domain_col] == z_Categories[0]],
                y_score=y_probs_lora[df_test[domain_col] == z_Categories[0], 1],
            )
        )
        auprc_lora_df1.append(
            metrics.average_precision_score(
                y_true=y_test[df_test[domain_col] == z_Categories[1]],
                y_score=y_probs_lora[df_test[domain_col] == z_Categories[1], 1],
            )
        )
        t = precision_recall_fscore_support(
            y_true=y_test,
            y_pred=y_probs_lora[:, 1] > 0.5,
            average="binary",
            pos_label=1,
        )
        t_df0 = precision_recall_fscore_support(
            y_true=y_test[df_test[domain_col] == z_Categories[0]],
            y_pred=y_probs_lora[df_test[domain_col] == z_Categories[0], 1] > 0.5,
            average="binary",
            pos_label=1,
        )
        t_df1 = precision_recall_fscore_support(
            y_true=y_test[df_test[domain_col] == z_Categories[1]],
            y_pred=y_probs_lora[df_test[domain_col] == z_Categories[1], 1] > 0.5,
            average="binary",
            pos_label=1,
        )
        precision_lora.append(t[0])
        recall_lora.append(t[1])
        f1_lora.append(t[2])
        precision_lora_df0.append(t_df0[0])
        recall_lora_df0.append(t_df0[1])
        f1_confounder_df0.append(t_df0[2])
        precision_lora_df1.append(t_df1[0])
        recall_lora_df1.append(t_df1[1])
        f1_lora_df1.append(t_df1[2])
...
df_eval = pd.DataFrame(
    {
        "auprc_lora": auprc_lora,
        "auprc_logistic_confounder_df0": auprc_lora_df0,
        "auprc_logistic_confounder_df1": auprc_lora_df1,
        "precision_lora": precision_lora,
        "recall_lora": recall_lora,
        "f1_lora": f1_lora,
        "precision_lora_df0": precision_lora_df0,
        "recall_lora_df0": recall_lora_df0,
        "f1_confounder_df0": f1_confounder_df0,
        "precision_lora_df1": precision_lora_df1,
        "recall_lora_df1": recall_lora_df1,
        "f1_lora_df1": f1_lora_df1,
