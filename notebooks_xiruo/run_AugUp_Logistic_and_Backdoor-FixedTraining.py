...
parser.add_argument(
    "--augMethod",
    type=str,
    choices=["mixup", "ReSample", "LLM_Generate", "noaug"],
    help="select sample configs",
)
...
parser.add_argument(
    ...
    type=float,
    default=1,
    help="alpha value for Beta(alpha, alpha) in mixup",
)
parser.add_argument(
    "--constraintCy",
    action="store_true",
...
from utilsAugment import augToN
...
from sample_unconstraintCy import SAMPLE_CONFIG
...
if args.constraintCy:
    ...
    sample_select = args.sample

    # sample_AlphaTrain = float(sample_select.split("-").split("_")[1])
    ...
    # p_pos_train_z0_ls = SAMPLE_CONFIG[sample_select]["p_pos_train_z0_ls"]
    # p_pos_train_z1_ls = SAMPLE_CONFIG[sample_select]["p_pos_train_z1_ls"]

    p_mix_z1_ls = SAMPLE_CONFIG[sample_select]["p_mix_z1_ls"]
    ...
    for combination in itertools.product(
        ...
        if round(
            combination[0] * (1 - combination[2]) + combination[1] * combination[2], 2
        ) not in [0.5, 0.4, 0.6, 0.3]:
            continue
...
###----------------- Fixed Training split & Train CLS
...
if transform == "binaryUnigram":
    ...
    _txts_full = list(dfs["train"][txt_col]) + list(dfs["test"][txt_col])

    if args.augMethod == "LLM_Generate":
        df_llm_expand = pd.read_csv(
            f"../output/LLM_Generate/{args.dataset}/expanded/sub-n_{n_test}-set_{CombinationIdx}.csv"
        )
        _txts_full = _txts_full + list(df_llm_expand[txt_col])

    _txts_full = [x for x in _txts_full if x is not np.nan]
    vectorizer.fit(_txts_full)

    df_training["embedding"] = list(
        vectorizer.transform(df_training[txt_col]).toarray()
    )
elif transform == "Sentence-BERT":
    ### use Sentence-BERT to encode sentences
    df_training["embedding"] = list(model.encode(df_training[txt_col]))
...
if args.augMethod != "noaug":
    ...
    def augDFmixup(df, name_to_aug):
        ...
        if n_augs < 0:
            ...
            x_generated = augToN(
                xIn=df["embedding"].apply(pd.Series).to_numpy(),
                augN=n_augs,
                alpha=alphaAug,
                seed=_rand,
            )

            # NOTE: assume df only has ONE domain category, ONE label category
            z_impute = df[domain_col].unique()[0]
            y_impute = df[label].unique()[0]

            _df = pd.DataFrame(
                {
                    "embedding": x_generated,
                    label: y_impute,
                    domain_col: z_impute,
                }
            )
    ...
    def augDFLLMGenerate(df, name_to_aug):
        n_augs = c_train_target[name_to_aug] - c[name_to_aug]

        df_llm_expand = pd.read_csv(
            f"../output/LLM_Generate/{args.dataset}/expanded/sub-n_{n_test}-set_{CombinationIdx}.csv"
        )
        df_llm_expand = df_llm_expand[df_llm_expand["ssid"].isin(df["ssid"])]
        ...
        if n_augs < 0:
            ...
            if transform == "binaryUnigram":
                _df["embedding"] = list(vectorizer.transform(_df[txt_col]).toarray())
            elif transform == "Sentence-BERT":
                ### use Sentence-BERT to encode sentences
                _df["embedding"] = list(model.encode(_df[txt_col]))
...
x_transform_train = df_training["embedding"].apply(pd.Series).to_numpy()

y_train = df_training[label]
z_train = df_training[domain_col]
...
outdir = f"../output/regression_Augmentation_{args.dataset}_BalanceAlpha/set-{CombinationIdx}/"
...
for iRun in range(runs):
    ...
    for c in tqdm(
        valid_full_settings,
        file=open(
            f"../log/test_Aug_{args.dataset}_{args.alphaAug}_ConstraintCy_{args.constraintCy}_{args.sample}.txt",
            "w",
        ),
    ):
...
if args.constraintCy:
    outname = f"{outdir}/ConstraintCy_mixup_{alphaAug}-AugUP-{transform}-ntest_{n_test}-{penalty}-C{C}-V{v}.pkl"
else:
    if args.augMethod == "mixup":
        fmethod = f"mixup_{alphaAug}"
    else:
        fmethod = args.augMethod

    outname = f"{outdir}/UnconstraintCy-{fmethod}-AugUP-{transform}-ntest_{n_test}-{penalty}-C{C}-V{v}_{args.sample}.pkl"
