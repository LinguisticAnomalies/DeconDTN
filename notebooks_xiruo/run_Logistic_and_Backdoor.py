...
outdir = f"../output/regression_{args.dataset}_BalanceAlpha"
...
for C, v in [
    ...
    for iRun in range(runs):
        ...
        for c in tqdm(
            valid_full_settings,
            file=open(
                f"../log/test_{args.dataset}_ConstraintCy_{args.constraintCy}.txt",
                "w",
            ),
        ):
    ...
    if args.constraintCy:
        outname = f"{outdir}/{transform}-ntest_{n_test}-{penalty}-C{C}-V{v}.pkl"
    else:
        outname = f"{outdir}/{transform}-ntest_{n_test}-{penalty}-C{C}-V{v}.pkl"
