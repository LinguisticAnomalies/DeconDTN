...
def confusion_matrix_probs(y_true, y_pred):
    ...
    idx = y_true == 0
    n_t0_p0 = sum(1 - y_pred[idx])
    n_t0_p1 = sum(y_pred[idx])

    n_t1_p0 = sum(1 - y_pred[~idx])
    n_t1_p1 = sum(y_pred[~idx])
...
for C, v in [
        ...
        for c in tqdm(
            ...
            if crossfitSplit != 0:
                ...
                for idx_train, idx_test in kf.split(x_transform_train):
                    ...
                    z_probs_IW_kfold.append(f0.predict_proba(X=x_transform_train_1))

                z_train_1_pred = np.mean(np.mean(z_probs_IW_kfold, axis=0), axis=0)
                ...
                # use second half to get Confusion Matrix c
                z_train_1_pred = np.mean(f0.predict_proba(x_transform_train_1), axis=1)
    ...
    if crossfitSplit != 0:
        outname = f"{outdir}/Importance_Weighting_forZ_Double-HyperDelta_{hyperparam_delta}-CrossFit_{crossfitSplit}-{transform}-ntest_{n_test}-{penalty}_C{C}-V{v}.pkl"
    else:
        outname = f"{outdir}/Importance_Weighting_forZ_Double-HyperDelta_{hyperparam_delta}-{transform}-ntest_{n_test}-{penalty}_C{C}-V{v}.pkl"
