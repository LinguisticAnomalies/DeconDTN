...
parser.add_argument(
    "--save_model",
    action="store_true",
    help="Save model after training finishes. Only for some specifications",
)
parser.add_argument(
    "--gpu",
    type=str,
...
globalconfig.per_device_train_batch_size = 64
...
df1 = df_shac_mimic
outdir = (
    f"../output/DistilBERT_SHACBalanceAlpha_10Pct_Fix_RandomPermute_ShorterVersion_1_5"
)
log_f = (
    "../log/DistilBERT_SHACBalanceAlpha_10Pct_Fix_RandomPermute_ShorterVersion_1_5.log"
)
globalconfig.fixed_pct = 0.1
globalconfig.permseed = 19
...
if args.save_model:
    ...
    outdir = f"../output/DistilBERT_SHACBalanceAlpha_10Pct_Fix_RandomPermute_SaveModel"
    log_f = "../log/DistilBERT_SHACBalanceAlpha_10Pct_Fix_RandomPermute_SaveModel.log"
...
for iRun in range(runs):
    ...
    for c in tqdm(valid_full_settings, file=open(log_f, "w")):
        ...
        if args.save_model:
            a_train = c["mix_param_dict"]["alpha_train"]
            a_test = c["mix_param_dict"]["alpha_test"]
            c_y = c["mix_param_dict"]["C_y"]
            outdir_specific_run = f"{outdir}/alpha_train__{a_train}__alpha_test__{a_test}__C_y__{c_y}__iRun__{iRun}.pth"
            trainer.save_model(outdir_specific_run)
