...
parser.add_argument(
    ...
    default="RS",
    choices=["RS", "S"],
    help="Project which matrix. Default is 'RS' - reverse source",
)
parser.add_argument(
    "--cpuOps",
    action="store_true",
...
weights_edited_file = f"{args.weightsEditedDir}/{os.path.basename(target_model_id)}-lambda1_{args.lambda1}-added-Norm-Proj_{args.projVecName}.pth"
    ...
    for key in key_list:
        ...
        if isinstance(target, LoraLayer):
            ...
            if projVecName == "RS":
                ...
                magnitude = norm_t / norm_s * (args.lambda1 - 1)
            elif projVecName == "S":
                norm_rs = torch.norm(torch.matmul(B_rs, A_rs), p="fro").item()
                magnitude = norm_t / norm_rs * (args.lambda1 - 1)
...
def ProjectionLoraWeights(
    ...
    for key in key_list:
        _, target, _ = _get_submodules(model_in.model, key)
        if isinstance(target, LoraLayer):
            if adapter_name not in target.lora_A:
                continue
            ...
            if projVecName == "RS":
                ...
                tmpv = np.vstack([B_rs.flatten().numpy(), B_t.flatten().numpy()])

            res = find_orth(tmpv.T)
            target.lora_B[adapter_name].weight.data = projVector(
                target.lora_B[adapter_name].weight.data, res
            )
...
if not args.DeltaFinished:
    ...
    if args.projVecName == "RS":
        ...
        model = amplifyLoraWeights(model_in=model, adapter_name="source", magnitude=1)

    lambda2_cls = args.lambda1 - 1
    #########################
    ##    Projection
    #########################
    if args.projVecName == "S":
        model = ProjectionLoraWeights(
            model_in=model,
            adapter_name="source",
            target_tensors=target_tensors,
            source_tensors=source_tensors,
            reverse_source_tensors=reverse_source_tensors,
            projVecName=args.projVecName,
        )
    ...
    if args.projVecName == "S":
        ...
        model = amplifyLoraWeights(
            model_in=model, adapter_name="source_reverse", magnitude=1
        )

    lambda3_cls = args.lambda1 - 1
    #########################
    ##    Projection
    #########################
    if args.projVecName == "RS":
