...
if args.constraintCy:
    ...
    for combination in itertools.product(
        ...
        if round(combination[1]/combination[0], 4) not in SAMPLE_CONFIG[sample_select]["alpha_train"]: # [1, 0.2, 5]: # [1, 2, 0.5, 3, 0.3333, 4, 0.25, 5, 0.2]:
            continue

        ## select based on Cy
        if round(combination[0]* (1-combination[2]) + combination[1]* combination[2], 4) not in SAMPLE_CONFIG[sample_select]["C_y"]:
            continue
...
outdir = f"../output/regression_AugUp_ReverseTranslate_{args.dataset}_BalanceAlpha"
...
for C, v in [
    ...
    for iRun in range(runs):
        ...
        for c in tqdm(
            valid_full_settings,
            file=open(
                f"../log/test_Aug_ReverseTranslate_{args.dataset}_ConstraintCy_{args.constraintCy}_{args.sample}.txt",
                "w",
            ),
        ):
            ...
            class NotEnoughToAugment(Exception):
                pass

            def augDF(df, df_aug_in, name_to_aug):
                ...
                if n_augs < 0:
                    ...
                    df_aug = deepcopy(df_aug_in)
                    df_aug = df_aug[df_aug["ssid"].isin(df["ssid"])]

                    if len(df_aug) < n_augs:
                        raise NotEnoughToAugment(
                            "Not Enough To Augment (len df < n aug)"
                        )
            ...
            df0_aug = pd.read_csv(
                f"../output/ReverseTranslate/{args.dataset}/df0_de.csv"
            )
            df1_aug = pd.read_csv(
                f"../output/ReverseTranslate/{args.dataset}/df1_de.csv"
            )
            df0_aug.drop(txt_col, axis=1, inplace=True)
            df1_aug.drop(txt_col, axis=1, inplace=True)
            df0_aug = df0_aug.rename(columns={"text_translated_reverse": txt_col})
            df1_aug = df1_aug.rename(columns={"text_translated_reverse": txt_col})

            try:
                df0_train_pos = augDF(
                    df0_train_pos, df0_aug[df0_aug[label] == 1], "n_z0_pos_train"
                )
                df0_train_neg = augDF(
                    df0_train_neg, df0_aug[df0_aug[label] == 0], "n_z0_neg_train"
                )
                df1_train_pos = augDF(
                    df1_train_pos, df1_aug[df1_aug[label] == 1], "n_z1_pos_train"
                )
                df1_train_neg = augDF(
                    df1_train_neg, df1_aug[df1_aug[label] == 0], "n_z1_neg_train"
                )
            except NotEnoughToAugment:
                continue
    ...
    if args.constraintCy:
        ...
        outname = f"{outdir}/UnconstraintCy-AugUP-ReSample-{transform}-ntest_{n_test}-{penalty}-C{C}-V{v}_{args.sample}.pkl"
