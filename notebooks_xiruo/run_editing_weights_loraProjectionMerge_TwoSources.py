...
weights_edited_file = f"{args.weightsEditedDir}/{os.path.basename(target_model_id)}-lambda1_{args.lambda1}-lambda2_{args.lambda2}-lambda3_{args.lambda3}-added-Proj_{args.projVecName}.pth"
...
if not args.DeltaFinished:
    ...
    model = PeftModel.from_pretrained(
        base_model, source_model_id_2, adapter_name="source_reverse", use_safetensors=False
    )
    score_weight_vector = (
